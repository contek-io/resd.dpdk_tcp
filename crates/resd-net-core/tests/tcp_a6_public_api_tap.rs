...
//!   `crates/resd-net/src/lib.rs`. Coverage of the `ApiTimer` variant
//!   shape + event-queue FIFO round-trip lives in the resd-net-core
...
//!   crates/resd-net/src/lib.rs (A5.5 Task 8). An ABI-level translator
//!   test belongs in crates/resd-net — flagged for Task 22/23 reviewers.
...
//! * Plan #7 (positive-path `resd_net_conn_rtt_histogram` via ABI) —
//!   deferred: requires a live DPDK/EAL Engine (negative-path / null-arg
//!   coverage lives in `resd_net_conn_rtt_histogram`'s existing unit tests
//!   in crates/resd-net/src/lib.rs — A6 Task 18).
...
//!   in crates/resd-net/src/lib.rs (Tasks 17, 18, 20); not duplicated here.
...
// `resd_net_timer_cancel` in crates/resd-net/src/lib.rs). Use it to
...
fn rtt_histogram_snapshot_is_disconnected_copy() {
    // `resd_net_conn_rtt_histogram` memcpys the bucket array into
...
fn ts_recent_fresh_conn_sentinel_never_expires() {
    ...
    let peer_opts = resd_net_core::tcp_options::TcpOpts {
...
fn ts_recent_recent_age_does_not_spuriously_expire() {
    ...
    c.ts_recent_age = resd_net_core::clock::now_ns();
